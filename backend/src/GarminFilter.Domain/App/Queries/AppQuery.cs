...
public record AppQuery(DeviceId DeviceId, AppType Type, bool? Paid, HashSet<AppPermission> ExcludePermissions, int PageIndex, int PageSize, AppOrder OrderBy)
...
file sealed class Handler : IRequestHandler<AppQuery, IEnumerable<AppAggregate>>
{
	...
	{
		var result = _repository.Query(request.DeviceId, request.Type, request.Paid, request.ExcludePermissions, request.PageIndex, request.PageSize, request.OrderBy);
